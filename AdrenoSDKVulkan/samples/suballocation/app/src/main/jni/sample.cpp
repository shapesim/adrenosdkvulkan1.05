...
// Sample Name
#define SAMPLE_NAME "Vulkan Sample: Suballocation"
...
void VkSample::InitVertexBuffers()
{
    ...
    {
        float xOffset = 0.0f;
        float yOffset = (i/NUM_COLS - (NUM_ROWS/2.0f) +0.5f) * 0.05f;
        float zOffset = (i%NUM_COLS - (NUM_COLS/2.0f) +0.5f) * 0.05f;
        if (!MeshObject::LoadObj(this, "diamond.obj", "diamond.mtl", 0.03f, -0.03f, 0.03f, xOffset, yOffset, zOffset, false, VERTEX_BUFFER_BIND_ID, &mMesh[i]))
        {
...
{
    VkShaderModule sh_normalmapping_vert = CreateShaderModuleFromAsset("shaders/suballocation.vert.spv");
    VkShaderModule sh_normalmapping_frag = CreateShaderModuleFromAsset("shaders/suballocation.frag.spv");
    ...
    // Use the first mesh to define the vertex input state
    VkPipelineVertexInputStateCreateInfo visci = mMesh[0].Buffer().CreatePipelineState();
...
void VkSample::BuildCmdBuffer()
{
    for (uint32_t i = 0; i < mSwapchainImageCount; i++)
    {
        ...
        vkCmdPushConstants(cmdBuffer, mPipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(mLight), &mLight);
        ...
        for (int i=0; i< NUM_OBJECTS; i++)
        {
            vkCmdBindVertexBuffers(cmdBuffer, VERTEX_BUFFER_BIND_ID, 1, &mMesh[i].Buffer().GetBuffer(), offsets);

            // Issue a draw command
            vkCmdDraw(cmdBuffer, mMesh[i].GetNumVertices(), 1, 0, 0);
        }
...
bool VkSample::InitSample()
{
    ...
    mVertUniformData.camPos = glm::vec3(0.8f, 0.0f, 0.0f);
    ...
    mLight.diffuse  = glm::vec4(1.0f, 0.2f, 0.2f, 1.0f);
    mLight.lightpos = glm::vec4(-1.0f, -1.5, -2.7f, 1.0f);
