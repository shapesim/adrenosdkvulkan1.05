...
// Sample Name
#define SAMPLE_NAME "Vulkan Tutorial: Monument"
...
void VkSample::InitVertexBuffers()
{
    ...
    for (uint32_t y = 0; y < gridSize; y++)
    {
        for (uint32_t x = 0; x< gridSize; x++)
        {
            ...
            float size = gridSize*scale*1.0001f;
            v.color = glm::vec4(0.5f, 0.5f, 0.5f, 1.0);
            v.uv.x = 1.0f-(v.pos.x/size);
            v.uv.y =  v.pos.z/size;
            vertices.push_back(v);
        }
    }

    mSurfaceVertices.InitBuffer(this, sizeof(vertex_layout) * vertices.size(), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, (const char*)&vertices[0]);
    mSurfaceVertices.AddBinding(VERTEX_BUFFER_BIND_ID, sizeof(vertex_layout), VK_VERTEX_INPUT_RATE_VERTEX);
    mSurfaceVertices.AddAttribute(VERTEX_BUFFER_BIND_ID, 0, 0, VK_FORMAT_R32G32B32_SFLOAT);                     // float3 position
    mSurfaceVertices.AddAttribute(VERTEX_BUFFER_BIND_ID, 1, sizeof(glm::vec4) , VK_FORMAT_R32G32B32A32_SFLOAT); // float4 color
    mSurfaceVertices.AddAttribute(VERTEX_BUFFER_BIND_ID, 2, sizeof(glm::vec4) * 2, VK_FORMAT_R32G32_SFLOAT);    // float2 uv
    ...
    mSurfaceIndexCount = indices.size();
    mSurfaceIndices.InitBuffer(this, sizeof(uint32_t) * indices.size(), VK_BUFFER_USAGE_INDEX_BUFFER_BIT, (const char*)&indices[0]);

    if (!MeshObject::LoadObj(this, "monument2.obj", "monument2.mtl", 4.0f, -4.0f, 4.0f, 0.0f, 0.0f, 0.0f, false, VERTEX_BUFFER_BIND_ID, &mMesh))
    {
...
{
    mMeshVertexUniformBuffer.InitBuffer(this, sizeof(mMeshVertUniformData), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, &mMeshVertUniformData);

    mStorageBuffer.InitBuffer(this, sizeof(mStorageData), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, &mStorageData);

    for (uint32_t i = 0; i < 4; i++)
    {
        mVertexUniformBuffer[i].InitBuffer(this, sizeof(mVertexUniform[0]), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, &mVertexUniform[i]);
    }
}
...
{
    bool success = true;
    // Load ASTC compressed textures
    success |= TextureObject::FromASTCFile(this, "vkbox.astc", &mTexVkLogo);
    success |= TextureObject::FromASTCFile(this, "tex.astc", &mTex);
    assert(success);
}
...
void VkSample::InitLayouts()
{
    ...
    uniformAndSamplerBinding[0].descriptorType      = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
...
void VkSample::InitPipelines()
{
    ...
    VkPipelineVertexInputStateCreateInfo visci = mSurfaceVertices.CreatePipelineState();
    ...
    InitPipeline(VK_NULL_HANDLE, 2, &shaderStages[0], mPipelineLayout, mRenderPass, 0, &visci, nullptr,
                 nullptr, nullptr, &rs, nullptr, nullptr, &cb, nullptr, false, VK_NULL_HANDLE, &mPipeline);
...
void VkSample::InitDescriptorSet()
{
    ...
    // 4 descriptors for 4 flag positions
    for (uint32_t i = 0; i < 4; i++)
    {
        err = vkAllocateDescriptorSets(mDevice, &descriptorSetAllocateInfo, &mDescriptorSet[i]);
        VK_CHECK(!err);
        ...
        //mvp
        VkDescriptorBufferInfo bufferInfo = mVertexUniformBuffer[i].GetDescriptorInfo();
...
void VkSample::BuildCmdBuffer()
{
    for (uint32_t i = 0; i < mSwapchainImageCount; i++)
    {
        ...
        clear_values[0].color.float32[1]        = 0.15f;
        ...
        vkCmdBindPipeline(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, mMeshPipeline);

        //bind out descriptor set, which handles our uniforms and samplers
        uint32_t dynamicOffsets[1]={0};
        vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, mMeshPipelineLayout, 0, 1, &mMeshDescriptorSet, 1, dynamicOffsets);
        ...
        // Bind our vertex buffer, with a 0 offset.
        offsets[1] = {0};
        vkCmdBindVertexBuffers(cmdBuffer, VERTEX_BUFFER_BIND_ID, 1, &mSurfaceVertices.GetBuffer(), offsets);

        vkCmdBindIndexBuffer(cmdBuffer, mSurfaceIndices.GetBuffer(), 0, VK_INDEX_TYPE_UINT32);

        for (uint32_t dIdx =0; dIdx < 4; dIdx++)
        {
            //bind out descriptor set, which handles our uniforms and samplers
            vkCmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, mPipelineLayout, 0,
                                    1, &mDescriptorSet[dIdx], 0, NULL);

            // Issue a draw command
            vkCmdDrawIndexed(cmdBuffer, mSurfaceIndexCount, 1, 0, 0, 0);
        }
...
bool VkSample::InitSample()
{
    ...
    glm::vec3 camera(-60.0f, -15.0f, -30.5f);
    glm::vec3 lookat(20.0f, -12.0f, 40.5f);
    glm::vec3 up(0.0f, 1.0f, 0.0f);
    mModelViewMatrix = glm::lookAtRH(camera, lookat, up);

    // This structure is used as a uniform in the rendering of compute shader output
    mVertexUniform[0].mvp = mProjectionMatrix * mModelViewMatrix
                            * glm::scale(glm::mat4(), glm::vec3(0.6f, 1.0f, 1.0f))
                            * glm::translate(glm::mat4(), glm::vec3(20.0f, -13.5f, -2.3f))
                            * glm::rotate(glm::mat4(), glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));

    mVertexUniform[1].mvp = mProjectionMatrix * mModelViewMatrix
                            * glm::scale(glm::mat4(), glm::vec3(0.6f, 1.0f, 1.0f))
                            * glm::translate(glm::mat4(), glm::vec3(-19.0f, -13.5f, -2.3f))
                            * glm::rotate(glm::mat4(), glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));

    mVertexUniform[2].mvp = mProjectionMatrix * mModelViewMatrix
                            * glm::scale(glm::mat4(), glm::vec3(0.6f, 1.0f, 1.0f))
                            * glm::translate(glm::mat4(), glm::vec3(-60.0f, -13.5f, -2.3f))
                            * glm::rotate(glm::mat4(), glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));

    mVertexUniform[3].mvp = mProjectionMatrix * mModelViewMatrix
                            * glm::translate(glm::mat4(), glm::vec3(-38.5f, -13.5f, 15.0f))
                            * glm::rotate(glm::mat4(), glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f))
                            * glm::rotate(glm::mat4(), glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));

    // This structure is used as a uniform in the rendering of the monument model
    mMeshVertUniformData.view = glm::lookAtRH(camera, lookat, up);
    mMeshVertUniformData.model = glm::mat4();
    mMeshVertUniformData.modelViewProjection = mProjectionMatrix * mMeshVertUniformData.view * mMeshVertUniformData.model;
...
void VkSample::InitComputeDescriptorSet()
{
    ...
    VkDescriptorBufferInfo vertBufferInfo =  mSurfaceVertices.GetDescriptorInfo();
    writes[0].sType                                 = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writes[0].dstBinding                            = 0;
    ...
    VkDescriptorBufferInfo detailBufferInfo =  mStorageBuffer.GetDescriptorInfo();
    writes[1].sType                                 = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writes[1].dstBinding                            = 1;
...
void VkSample::InitMeshLayouts()
{
    ...
    descriptorSetLayoutCreateInfo.pNext                 = nullptr;
    descriptorSetLayoutCreateInfo.bindingCount          = 2;
    descriptorSetLayoutCreateInfo.pBindings             = &descriptorSetLayoutBinding[0];

    ret = vkCreateDescriptorSetLayout(mDevice, &descriptorSetLayoutCreateInfo, nullptr, &mMeshDescriptorLayout);
    VK_CHECK(!ret);
    ...
    pipelineLayoutCreateInfo.pNext                      = nullptr;
    pipelineLayoutCreateInfo.setLayoutCount             = 1;
    pipelineLayoutCreateInfo.pSetLayouts                = &mMeshDescriptorLayout;

    ret = vkCreatePipelineLayout(mDevice, &pipelineLayoutCreateInfo, nullptr, &mMeshPipelineLayout);
    VK_CHECK(!ret);
}

///////////////////////////////////////////////////////////////////////////////

void VkSample::InitMeshDescriptorSet()
{
    //Create a pool with the amount of descriptors we require
    VkDescriptorPoolSize poolSize[2] = {};
    ...
    VkResult  err;
    err = vkCreateDescriptorPool(mDevice, &descriptorPoolCreateInfo, NULL, &mMeshDescriptorPool);
    VK_CHECK(!err);
    ...
    descriptorSetAllocateInfo.sType                 = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    descriptorSetAllocateInfo.pNext                 = nullptr;
    descriptorSetAllocateInfo.descriptorPool        = mMeshDescriptorPool;
    descriptorSetAllocateInfo.descriptorSetCount    = 1;
    descriptorSetAllocateInfo.pSetLayouts = &mMeshDescriptorLayout;

    err = vkAllocateDescriptorSets(mDevice, &descriptorSetAllocateInfo, &mMeshDescriptorSet);
    VK_CHECK(!err);

    VkWriteDescriptorSet writes[2] = {};

    VkDescriptorBufferInfo vertUniform = mMeshVertexUniformBuffer.GetDescriptorInfo();
    writes[0].sType             = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writes[0].dstBinding        = 0;
    writes[0].dstSet            = mMeshDescriptorSet;
    writes[0].descriptorCount   = 1;
    ...
    descriptorImageInfo.sampler     = mTex.GetSampler();
    descriptorImageInfo.imageView   = mTex.GetView();
    descriptorImageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;

    // texture
    writes[1].sType                 = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    writes[1].dstBinding            = 1;
    writes[1].dstSet                = mMeshDescriptorSet;
    writes[1].descriptorCount       = 1;
    writes[1].descriptorType        = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    writes[1].pImageInfo            = &descriptorImageInfo;
...
void VkSample::InitMeshPipelines()
{
    VkShaderModule sh_model_vert = CreateShaderModuleFromAsset("shaders/model.vert.spv");
    VkShaderModule sh_model_frag = CreateShaderModuleFromAsset("shaders/model.frag.spv");
    ...
    VkSampleFramework::InitPipeline(VK_NULL_HANDLE, &visci, mMeshPipelineLayout, mRenderPass,
                                    &rs, nullptr, sh_model_vert, sh_model_frag, false, VK_NULL_HANDLE, &mMeshPipeline);

    vkDestroyShaderModule(mDevice, sh_model_vert, nullptr);
    vkDestroyShaderModule(mDevice, sh_model_frag, nullptr);
}
...
bool VkSample::Update()
{
    ...
    ret = vkMapMemory(mDevice, mStorageBuffer.GetDeviceMemory(), 0, mStorageBuffer.GetAllocationSize(), 0, (void **) &pData);
    assert(!ret);

    mStorageData.time = time;

    memcpy(pData, &mStorageData, sizeof(mStorageData));

    vkUnmapMemory(mDevice, mStorageBuffer.GetDeviceMemory());
...
bool VkSample::DestroySample()
{
    ...
    vkDestroyDescriptorSetLayout(mDevice, mComputeDescLayout, nullptr);
    
    vkDestroyPipeline(mDevice, mMeshPipeline, nullptr);
    vkDestroyPipelineLayout(mDevice, mMeshPipelineLayout, nullptr);
    vkDestroyDescriptorSetLayout(mDevice, mMeshDescriptorLayout, nullptr);

    vkDestroyDescriptorPool(mDevice, mDescriptorPool,nullptr);

    vkFreeCommandBuffers(mDevice, mComputeCommandPool, 1, &mComputeCmdBuffer);
    vkDestroyCommandPool(mDevice, mComputeCommandPool, nullptr);

    // Destroy verts
    mSurfaceVertices.Destroy();
    mSurfaceIndices.Destroy();
    mMesh.Destroy();

    // Destroy uniform
    for (uint32_t i = 0; i < 4; i++)
    {
        mVertexUniformBuffer[i].Destroy();
    }
    
    mMeshVertexUniformBuffer.Destroy();
