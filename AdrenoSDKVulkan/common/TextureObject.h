...
public:
    ...
    static bool FromKTXFile(VkSampleFramework* sample, const char* filename, TextureObject* pTextureObject);

    static bool FromTGAFile(VkSampleFramework* sample, const char* filename, TextureObject* textureObject);

    static bool FromTGAImageData(VkSampleFramework *sample, TextureObject *textureObject, uint32_t *imageData);
...
protected:
    ...
    static TKTXErrorCode LoadKTXFromMemory(VkSampleFramework* sample, const uint8_t* dataInMemory, uint32_t filesize, TextureObject* textureObject);
