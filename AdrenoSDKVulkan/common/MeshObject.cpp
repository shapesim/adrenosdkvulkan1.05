...
MeshObject::MeshObject() : mSample(nullptr), mNumVertices(0)
{
...
bool MeshObject::LoadObj(VkSampleFramework* sample, const char* objFilename, const char* mtlFilename,
                         ...
                         MeshObject* meshObject)
{
    ...
    AAsset* materialFile = AAssetManager_open(sample->GetAssetManager(), mtlFilename, AASSET_MODE_BUFFER);
    assert(materialFile);
    size_t matsize = AAsset_getLength(materialFile);

    const char* objectFileData = (const char*)AAsset_getBuffer(objectFile);
    std::stringstream objectFileStream(objectFileData);

    const char* materialFileData = (const char*)AAsset_getBuffer(materialFile);

    MaterialStringStreamReader materialFileStream(materialFileData);
    ...
    for (size_t i = 0; i < shapes.size(); i++)
    {
        ...
        for (size_t f = 0; f < shapes[i].mesh.indices.size() / 3; f++)
        {
            ...
            for (uint32_t idx = 0; idx < 3; idx++)
            {
                ...
                vertex_layout vertex;
                vertex.pos[0] =  (shapes[i].mesh.positions[3 * v    ] * xScale + xOffset);
                vertex.pos[1] =  (shapes[i].mesh.positions[3 * v + 1] * yScale + yOffset);
                vertex.pos[2] =  (shapes[i].mesh.positions[3 * v + 2] * zScale + zOffset);
                ...
                // Calculate min/max of vertices
                if (objVertices.size() ==0)
                {
                    for (uint32_t i=0; i<3; i++)
                    {
                        meshObject->minPos[i] = vertex.pos[i];
                        meshObject->maxPos[i] = vertex.pos[i];
                    }
                }
                else
                {
                    for (uint32_t i=0; i<3; i++)
                    {
                        meshObject->minPos[i] = fmin(meshObject->minPos[i], vertex.pos[i]);
                        meshObject->maxPos[i] = fmax(meshObject->maxPos[i], vertex.pos[i]);
                    }
                }
                objVertices.push_back(vertex);
            }
    ...
    {
        float offset[3];
        offset[0] = -(meshObject->minPos[0] + meshObject->maxPos[0])*0.5f;
        offset[1] = -(meshObject->minPos[1] + meshObject->maxPos[1])*0.5f;
        offset[2] = -(meshObject->minPos[2] + meshObject->maxPos[2])*0.5f;

        for (int v=0; v<objVertices.size(); v++)
        {
            for (uint32_t i=0; i<3; i++)
            {
                objVertices[v].pos[i] += offset[i];
            }
        }
    }
